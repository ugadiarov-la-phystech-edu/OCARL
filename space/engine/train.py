...
from space.solver import get_optimizers
from space.utils import Checkpointer, MetricLogger
...
def train(cfg):
    ...
    if cfg.seed  <= 0:
      cfg.seed = np.random.randint(1, 100000000)
    ...
    for epoch in range(start_epoch, cfg.train.max_epochs):
        ...
        for i, data in enumerate(trainloader):
            ...
            if (global_step) % cfg.train.print_every == 0:
                ...
                vis_logger.train_vis(model, writer, log, global_step, 'train')
                vis_logger.cat_vis(model, writer, global_step, cfg.device)
                end = time.perf_counter()
