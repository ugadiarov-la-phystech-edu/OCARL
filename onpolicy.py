...
def onpolicy_trainer(
    ...
    if resume_from_log:
        ...
        print('RESUME for log. +++++++++++++++++++++++++++++')
        print(start_epoch, env_step, gradient_step)
    if start_infos is not None:
      start_epoch, env_step, gradient_step = start_infos
    ...
    if custom_test_episode is None:
      cur_test_episode = test_episode
    else:
      cur_test_episode = custom_test_episode
    test_result = cur_test_episode(
    ...
    for epoch in range(1 + start_epoch, 1 + max_epoch):
        ...
        with tqdm.tqdm(
            ...
            max_ret, min_ret = -100000, 100000
            level_max = np.ones(200) * max_ret
            level_len = np.ones(200) * 0
            level_mean = np.zeros(200)
            level_num = np.ones(200)
            while t.n < t.total:
                if train_fn:
                ...
                if result["n/ep"] > 0:
                    L.record_mean('train_ret/mean', result['rew'])
                    L.record_mean('train_ret/std', result['rew_std'])
                    rmax, rmin = result['rews'].max(), result['rews'].min()
                    max_ret = max_ret if max_ret > rmax else rmax 
                    min_ret = min_ret if min_ret < rmax else rmin 
                    for k in result:
                      if k.startswith('metric'):
                        L.record_mean(k, result[k])
        ...
        if L is not None:
          ...
          L.record('train/collect_time', train_collector.collect_time)
          L.record('test/collect_time', test_collector.collect_time)
          L.record('train/train_time', time.time() - start_time - test_collector.collect_time - train_collector.collect_time)
          for k in test_result:
            if 'SepTest' in k:
              L.record(k, test_result[k])
            if k.startswith('metric'):
              L.record(k, test_result[k])
          L.dump(step=env_step)
        if verbose:
            print(
