...
class Collector(object):
    ...
    def reset_env(self) -> None:
        ...
        if self.env_reset_fn is not None:
          self.env = self.env_reset_fn()
        obs = self.env.reset()
    ...
    def collect(
        ...
        while True:
            ...
            if random:
                ...
                if 'new_obs' in result:
                  self.data.update(obs=result.new_obs)
                  ...
                  for ep_info in episode_infos:
                    for k in ep_info:
                      if k.startswith('metric'):
                        if k not in other_metrics:
                          other_metrics[k] = []
                        other_metrics[k].append(ep_info[k])
        ...
        if len(other_metrics) > 0:
          for k, v in other_metrics.items():
            # return_info[k] = v
            return_info[f'{k}_mean'] = np.mean(v)
            return_info[f'{k}_std'] = np.std(v)
        return return_info
